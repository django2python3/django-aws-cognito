...
if DJANGO_VERSION[1] > 10:
    class CognitoBackend(AbstractCognitoBackend):
        def authenticate(self, request, username=None, password=None):
            ...
            """
            user = super(CognitoBackend, self).authenticate( username=username, password=password)
            if user:
        ...
        def authenticate(self, request, username=None, password=None):
            ...
            if user:
                ...
                try:
                    ...
                    token_obj.access_token = user.access_token
                    token_obj.id_token = user.id_token
                    token_obj.refresh_token = user.refresh_token
                    token_obj.save()
                except Token.DoesNotExist:
                    token_obj = Token.objects.create(user = user, access_token = user.access_token, id_token = user.id_token, refresh_token = user.refresh_token)
            return user
            # return super(CognitoBackend, self).authenticate( username=username, password=password)
